...
class WorkoutForm extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      seconds:
        ("0" + Math.round((props.workout.duration % 1) * 60)).slice(-2) || "",
      distance: 0,
    };
  ...
  componentDidMount() {
    this.props.fetchAllRoutes().then((payload) => {
  ...
  updateIntegerTime(field) {
    return (e) => {
      const newInt = parseInt(
        ("0" + parseInt(e.currentTarget.value, 10)).slice(-2)
      );
  ...
  routesDropdown() {
    if (!this.props.routes) {
      return (
        <select className="numbered-select">
          <option></option>
        </select>
      );
    }
    ...
    return (
      <div className="workout-form">
        <div className="workouts-sub-header">
          ...
          <div className="workout-errors">{errors.join(" | ")}</div>
        </div>
